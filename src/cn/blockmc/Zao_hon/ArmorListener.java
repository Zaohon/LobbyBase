...
import cn.blockmc.Zao_hon.ArmorEquipEvent.EquipMethod;
...
public class ArmorListener implements Listener {
	...
	public void playerInteractEvent(PlayerInteractEvent e) {
		...
		if (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) {
			...
			if (e.getClickedBlock() != null && e.getAction() == Action.RIGHT_CLICK_BLOCK) {// Having
																							...
																							// it
																							// though.
				// Some blocks have actions when you right click them which
				// stops the client from equipping the armor in hand.
	...
	public void inventoryDrag(InventoryDragEvent event) {
		 ...
		 * event.getCursor().getType().name()); } if(event.getOldCursor() !=
		 * null){ System.out.println("OldCursor: " +
		 * event.getOldCursor().getType().name()); } System.out.println("Type: "
		 * + event.getType().name());
		 */
	}

	/*
	 * @EventHandler public void dispenserFireEvent(BlockDispenseEvent e){
	 * ArmorType type = ArmorType.matchType(e.getItem()); if(type != null){
	 * org.bukkit.block.Dispenser dispenser = (org.bukkit.block.Dispenser)
	 * e.getBlock().getState(); org.bukkit.material.Dispenser dispenserData =
	 * (org.bukkit.material.Dispenser) dispenser.getData(); Location loc =
	 * shift(e.getBlock().getLocation(), dispenserData.getFacing(), 1);
	 * List<EntityLiving> list = ((CraftWorld)
	 * loc.getWorld()).getHandle().a(EntityLiving.class, new AxisAlignedBB(new
	 * BlockPosition(loc.getX(), loc.getY(), loc.getZ())),
	 * Predicates.and(IEntitySelector.e, new
	 * IEntitySelector.EntitySelectorEquipable(CraftItemStack.asNMSCopy(e.
	 * getItem())))); if(list.isEmpty()) return; EntityLiving ent = list.get(0);
	 * if(ent instanceof EntityPlayer){ Player p = (Player)
	 * ent.getBukkitEntity(); ArmorEquipEvent armorEquipEvent = new
	 * ArmorEquipEvent(p, EquipMethod.DISPENSER, type, null, e.getItem());
	 * Bukkit.getServer().getPluginManager().callEvent(armorEquipEvent);
...
//		if (multiplier == 0)
//			return start;
//		return new Location(start.getWorld(), start.getX() + direction.getModX() * multiplier,
//				start.getY() + direction.getModY() * multiplier, start.getZ() + direction.getModZ() * multiplier);
//	}
}
